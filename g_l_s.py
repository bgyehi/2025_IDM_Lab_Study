...
def build_and_solve_gurobi(p, d, time_limit=1800, mip_gap=0.01, threads=8):
    ...
    model.Params.MIPFocus = 3          # Bound 개선 중심
    model.Params.Cuts = 3              # 절단평면 최대
    model.Params.Presolve = 2          # 사전 단순화 최대
    model.Params.Heuristics = 0.3      # 초기해 탐색 강화
    model.Params.VarBranch = 2         # Strong branching
    ...
    start_time = 0
    for j in order:
        S[j].start = start_time
        start_time += p[j]
        T[j].start = max(0, start_time - d[j])
...
if __name__ == "__main__":
    ...
    if "total_tardy" in sol:
        print(f"Gurobi total tardiness = {sol['total_tardy']:.2f}, Time = {gurobi_time:.2f} sec")
    else:
        print(sol)
