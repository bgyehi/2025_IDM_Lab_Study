...
def generate_instance(n=100, p_low=1, p_high=100, tightness=0.45, seed=None):
    ...
    # 각 job의 due-date = random 범위 + 프로세싱 타임
...
def tune_tightness_for_target(n=100, target_frac=0.5, tol=0.05, max_iters=20, seed=123):
    """
    SPT 기준 tardy 비율이 target_frac에 가깝도록 tightness 조정
    반환: p, d, tightness, 실제 tardy fraction
    """
    low, high = 0.05, 1.0
...
if __name__ == "__main__":
    n = 100
    # 자동 튜닝으로 '아슬아슬' 인스턴스 생성 (목표 tardy 비율 ≈ 0.5)
    ...
    # Gurobi로 최적화 (TimeLimit=1800초)
    sol = build_and_solve_gurobi(p, d, time_limit=1800, mip_gap=0.01, threads=0)
...
# Insert 완전 탐색 로컬 서치
# ----------------------------
def insert_full_search(seq):
    best_seq = seq[:]
    best_tardiness = total_tardiness(best_seq)

    for i in range(len(seq)):
        for j in range(len(seq)):
            if i != j:
                new_seq = insert(best_seq, i, j)
                new_tardiness = total_tardiness(new_seq)
...
best_insert_seq, best_insert_tardiness = insert_full_search(current_seq)
print("Insert Full Search Best Tardiness:", best_insert_tardiness)
...
# ----------------------------
# Block Reverse 완전 탐색 로컬 서치
# ----------------------------
def block_full_search(seq):
    best_seq = seq[:]
    best_tardiness = total_tardiness(best_seq)
...
best_block_seq, best_block_tardiness = block_full_search(current_seq)
print("Block Reverse Full Search Best Tardiness:", best_block_tardiness)
